...
#tf.app.flags.DEFINE_string('data_dir', 'F:\\code\\',
#                           """Path to the CIFAR-10 data directory.""")
tf.app.flags.DEFINE_string('data_dir', '/home/tuxiang/桌面/code',
                           """Path to the CIFAR-10 data directory.""")
...
def cnn_model(input_images):
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        with tf.variable_scope('conv1'):
            ...
            tf.summary.image('conv1/filters', kernel_transposed, max_outputs=3,collections=None)
    #打印出特征图
    #print(kernel)
    #print(conv1)
    convt = tf.slice(conv1,[0,0,0,0],[24,24,3,64])
   # print(convt)
    with tf.variable_scope('conv1'):
        # scale weights to [0 1], type is still float
        x_min = tf.reduce_min(convt)
        x_max = tf.reduce_max(convt)
        kernel_0_to_1 = (convt - x_min) / (x_max - x_min)

        # to tf.image_summary format [batch_size, height, width, channels]
        kernel_transposed = tf.transpose(kernel_0_to_1, [3, 0, 1, 2])

        # this will display random 3 filters from the 64 in conv1
        tf.summary.image('conv1/features', kernel_transposed, max_outputs=3, collections=None)
